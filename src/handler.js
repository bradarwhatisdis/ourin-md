...
async function getSmartTriggerThumb() {
  ...
  try {
    const buf = getAssetBuffer("ourin2");
    if (buf) {
      _smartTriggerThumb = buf;
    } else {
    ...
    for (let participant of participants) {
      ...
      const rankActions = {
        promote: {
          ...
          imgKey: "_promoteImg",
          imgPath: getAssetBuffer("ourin-promote"),
          emoji: "🎉",
          label: "PROMOTE",
          text: (p, a) =>
        ...
        demote: {
          ...
          imgKey: "_demoteImg",
          imgPath: getAssetBuffer("ourin-demote"),
          emoji: "📉",
          label: "DEMOTE",
          text: (p, a) =>
      ...
      if (rankCfg && groupData[rankCfg.notifKey] === true) {
        ...
        if (!groupHandler[rankCfg.imgKey]) {
          try {
            groupHandler[rankCfg.imgKey] = Buffer.isBuffer(rankCfg.imgPath) ? rankCfg.imgPath : fs.readFileSync(rankCfg.imgPath);
          } catch {
